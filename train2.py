...
import torch
from torch.amp import GradScaler  # Use torch.amp instead of torch.cuda.amp for AMP
...
from core.utils.torch_utils import (
    count_model_params,
    sync_nodes,  # Import sync_nodes but it will be skipped for MPS
    cleanup,  # Import cleanup but it will be skipped for MPS
    save_ckpt,
    load_ckpt,
    get_lr,
    get_optimizer,
...
class Train:
    def __init__(self, args):
        ...
        self.is_ddp = False  # Distributed training is not supported with MPS
        ...
        self.log(f"Running on device = {self.device}")
        ...
        self.mixed_precision = False  # Disable mixed precision for MPS
        ...
        # Init model
        self.model = get_model(cfg, self.device, is_ddp=False, compile=self.compile)
        self.optimizer = get_optimizer(self.model, cfg.optimizer)
        self.scheduler = get_scheduler(
            self.optimizer, cfg.train.warmup_iters, cfg.optimizer.lr_decay
        )
        self.scaler = GradScaler(enabled=False)  # Disable GradScaler for MPS
    ...
    def run_training(self):
        ...
        if self.load_ckpt_path is not None:
            self.model = load_ckpt(
                self.load_ckpt_path, self.model, is_ddp=False, compile=self.compile
            )[0]
