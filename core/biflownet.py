...
class ImportanceMask(nn.Module):
    def __init__(self, mode):
        ...
        self.conv_img = ConvBlock(in_channels=1, out_channels=4, kernel_size=3, padding_mode=mode, act=None)
        self.conv_error = ConvBlock(in_channels=1, out_channels=4, kernel_size=3, padding_mode=mode, act=None)
...
class MaskUNet(nn.Module):
    def __init__(self, chs_in, num_channels, padding_mode='zeros'):
        ...
        self.down2 = conv2(2*num_channels, 4*num_channels, padding_mode=padding_mode)
        self.down3 = conv2(4*num_channels, 8*num_channels, padding_mode=padding_mode)
        self.down4 = conv2(8*num_channels, 16*num_channels, padding_mode=padding_mode)
        self.up1 = deconv(16*num_channels, 8*num_channels, padding_mode=padding_mode)
...
class FeatPyramid(nn.Module):
    ...
    def __init__(self, num_channels=24, kernel_size=3, mode="zeros"):
        ...
        self.conv_stage1 = nn.Sequential(
            ...
            *[ConvBlock(num_channels, num_channels, kernel_size=kernel_size, padding_mode=mode, act=act) for _ in range(2)]
        )
        self.conv_stage2 = nn.Sequential(
            ConvBlock(num_channels, 2*num_channels, kernel_size=2, padding_mode=mode, act=act),
            *[ConvBlock(2*num_channels, 2*num_channels, kernel_size=kernel_size, padding_mode=mode, act=act) for _ in range(5)]
        )
...
class Estimator(nn.Module):
    ...
    def __init__(self, pyr_dim=24, kernel_size=3, corr_radius=4, mode="zeros"):
        ...
        in_channels = (corr_radius*2 + 1) ** 2 + image_feat_channel * 2 + last_flow_feat_channel + 4

        self.corr = Correlation(corr_radius)

        act = 'lrelu'
        self.convs = nn.Sequential(
            ConvBlock(in_channels=in_channels, out_channels=160, kernel_size=1, padding_mode=mode, act=act),
            ConvBlock(in_channels=160, out_channels=128, kernel_size=kernel_size, padding_mode=mode, act=act),
            ConvBlock(in_channels=128, out_channels=112, kernel_size=kernel_size, padding_mode=mode, act=act),
            ConvBlock(in_channels=112, out_channels=96, kernel_size=kernel_size, padding_mode=mode, act=act),
            ConvBlock(in_channels=96, out_channels=64, kernel_size=kernel_size, padding_mode=mode, act=act)    
        )

        self.final_conv = ConvBlock(in_channels=64, out_channels=4, kernel_size=kernel_size, padding_mode=mode, act=None)

    def forward(self, feat0, feat1, last_feat, last_flow):
        volume = F.leaky_relu(input=self.corr(feat0, feat1), negative_slope=0.1, inplace=False)
...
class ForwardWarp(nn.Module):
    ...
    def fn_avg_splat(self, img0, img1, flow):
        img0 = warp_fn(tenInput=img0, tenFlow=flow[:, :2]*0.5, tenMetric=None, strType='average')
        img1 = warp_fn(tenInput=img1, tenFlow=flow[:, 2:]*0.5, tenMetric=None, strType='average')
        return img0, img1

    def fn_fw_splat(self, img0, img1, flow):
        img0 = warp_fn(tenInput=img0, tenFlow=flow[:, :2]*1.0, tenMetric=None, strType='average')
        img1 = warp_fn(tenInput=img1, tenFlow=flow[:, 2:]*1.0, tenMetric=None, strType='average') 
        return img0, img1

    def fn_soft_splat(self, img0, img1, flow):
        tenMetric0 = F.l1_loss(input=img0, target=backwarp(tenInput=img1, tenFlow=flow[:, :2]*0.5), reduction='none').mean([1], True)
        tenMetric0 = self.alpha(img0, -tenMetric0).neg().clip(-20.0, 20.0)
        img0 = warp_fn(tenInput=img0, tenFlow=flow[:, :2]*0.5, tenMetric=tenMetric0, strType='softmax')
        
        tenMetric1 = F.l1_loss(input=img1, target=backwarp(tenInput=img0, tenFlow=flow[:, 2:]*0.5), reduction='none').mean([1], True)
...
class BiFlowNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.feat_pyramid = FeatPyramid(args.pyr_dim, args.kernel_size, args.padding_mode)
        self.flow_estimator = Estimator(args.pyr_dim, args.kernel_size, args.corr_radius, args.padding_mode)
    ...
    def pre_warp(self, img0, img1, last_flow):
        up_flow = F.interpolate(input=last_flow, scale_factor=4.0, mode="bilinear", align_corners=False) * 4
    ...
    def forward(self, img0, img1):
        ...
        img0_down = F.interpolate(input=img0, scale_factor=scale_factor, mode="bilinear", align_corners=False)
        img1_down = F.interpolate(input=img1, scale_factor=scale_factor, mode="bilinear", align_corners=False)

        last_flow = torch.zeros((N, 4, H // (2 ** (level+2)), W //(2 ** (level+2))), device=img0.device)
        last_feat = torch.zeros((N, self.last_flow_feat_channel, H // (2 ** (level+2)), W // (2 ** (level+2))), device=img0.device)

        flow, feat = self.forward_one_iteration(img0_down, img1_down, last_feat, last_flow)
        ######
        ...
        for level in list(range(self.pyr_level-1))[::-1]:
            ...
            last_flow = F.interpolate(input=flow, scale_factor=2.0, mode="bilinear", align_corners=False) * 2
            last_feat = F.interpolate(input=feat, scale_factor=2.0, mode="bilinear", align_corners=False)
        ...
        output_flow = F.interpolate(input=flow, scale_factor=4.0, mode="bilinear", align_corners=False) * 4
