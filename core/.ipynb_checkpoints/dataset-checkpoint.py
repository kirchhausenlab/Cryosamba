...
class DatasetBase(Dataset):
    def __init__(self, args, data, metadata, split="train"):
        ...
        self.indices = [np.arange(-overlap, shape, patch_shape-overlap) for overlap, shape, patch_shape in zip(self.patch_overlap, self.shape, self.patch_shape)]
        self.indices[0] = np.arange(0, self.shape[0]-self.patch_overlap[0], self.patch_shape[0]-self.patch_overlap[0])
        ...
        self.dataset_length = self.index_shape[0]*self.index_shape[1]*self.index_shape[2]

        self.coords_list, self.border_pad_list = list(zip(*[self.get_crop_params(index) for index in range(self.dataset_length)]))
    ...
    def get_crop_params(self, index):
        ...
        coords_start = np.asarray([self.indices[i][idx] for i, idx in enumerate(index_unravel)]).astype('int')
        coords_end = np.asarray([coord + shape for coord, shape in zip(coords_start, self.patch_shape)]).astype('int')
    ...
    def __getitem__(self, index):
        ...
        imgs = self.data[coords[0,0]:coords[0,1], coords[1,0]:coords[1,1], coords[2,0]:coords[2,1]]
        imgs = np.pad(imgs, border_pad, mode='reflect')
        imgs = torch.from_numpy(imgs).float()
        
        if self.split=='train': imgs = augment_dataset(imgs)
...
def get_dataloader(args, data_list, metadata_list, split, is_ddp, shuffle=False):
    dataset = ConcatDataset([DatasetBase(args, data, metadata, split=split) for data, metadata in zip(data_list, metadata_list)])

    sampler = DistributedSampler(dataset, shuffle=shuffle) if is_ddp else None
    shuffle = None if is_ddp else shuffle

    return DataLoader(dataset, batch_size=args.batch_size, pin_memory=True, num_workers=args.num_workers, drop_last=False, sampler=sampler, shuffle=shuffle)
