...
from core.utils.data_utils import get_data, unpad3D, save_data, denormalize_imgs, get_overlap_pad
from core.utils.torch_utils import setup_DDP, sync_nodes, count_model_params, cleanup, load_ckpt

class Inference:
    def __init__(self, args, gap):
        self.run_stamp = time()
        ...
        ##############################
        cfg.inference_data.max_frame_gap = gap
        cfg.inference_dir = "/nfs/datasync4/inacio/data/denoising/cryosamba/benchmark/virus_1x/gap_"+str(gap).zfill(2)
        
        if self.rank==0: 
            setup_run(cfg, mode="inference")
        sync_nodes(self.is_ddp)
            
        self.log(f'Using seed number {args.seed}' if args.seed!=-1 else f'No random seed was set')
        ...
        self.overlap_pad = get_overlap_pad(cfg.inference_data.patch_overlap, self.device)
        ...
        ckpt_name = "last" if cfg.inference.load_ckpt_name is None else cfg.inference.load_ckpt_name
        ckpt_path = os.path.join(cfg.train_dir, ckpt_name + '.pt')
        if not os.path.exists(ckpt_path):
            raise ValueError(f"Model checkpoint {ckpt_path} does not exist")
            
        self.model = get_model(cfg_train, self.device, is_ddp=self.is_ddp, compile=self.compile)
        self.model = load_ckpt(ckpt_path, self.model, is_ddp=self.is_ddp, compile=self.compile)[0]
        self.model.eval()
        ...
        data_list, metadata_list = list(zip(*[get_data(path) for path in cfg.data_path]))
        self.inference_loader = get_dataloader(cfg.inference_data, data_list, metadata_list, split="test", is_ddp=self.is_ddp)
        ...
        sync_nodes(self.is_ddp)
        self.output_array = np.memmap(self.output_temp_name, dtype=self.metadata["dtype"], mode='r+', shape=self.metadata["shape"])
    ...
    def make_output_temp_file(self):
        if not os.path.exists(self.output_temp_name):
            if self.rank==0:
                output_array = np.memmap(self.output_temp_name, dtype=self.metadata["dtype"], mode='w+', shape=self.metadata["shape"])
    ...
    def skip_iter(self, imgs, z, out_coords):
        output_mimmax = min([self.output_array[z[j], out_coords[j,1,0]:out_coords[j,1,1], out_coords[j,2,0]:out_coords[j,2,1]].max() for j in range(imgs.shape[0])])
        return True if output_mimmax!=0.0 else False

    def TTA_transforms(self, x):
        if self.TTA:
            return [x[0], x[1].flip(dims=[-1]), x[2].flip(dims=[-2]), x[3].flip(dims=[-1,-2])]
        else:
            return x
    ...
    def run_inference(self):
        ...
        for i, [imgs, crop_params] in enumerate(self.inference_loader):
            ...
            imgs, crop_params = imgs.to(device=self.device), crop_params.to(device=self.device)
            pad, out_coords, z = self.process_crop_params(crop_params)
            if self.skip_iter(imgs, z, out_coords): continue

            imgs = torch.split(imgs, 1, dim=1)

            recs = []
            for t in range(1, self.max_frame_gap+1):
                img0, imgT, img1 = imgs[self.max_frame_gap-t].contiguous(), imgs[self.max_frame_gap].contiguous(), imgs[self.max_frame_gap+t].contiguous()
            ...
            for j in range(rec.shape[0]):
                self.output_array[z[j], out_coords[j,1,0]:out_coords[j,1,1], out_coords[j,2,0]:out_coords[j,2,1]] = unpad3D(rec[j], pad[j])
    
            self.log(f"Iter {i}/{len(self.inference_loader)}, Elapsed time = {(time()-iter_time):.3f}")
            self.output_array.flush()
    
        save_data(path=self.inference_dir, name="result", data=self.output_array, metadata=self.metadata, output_format=self.output_format)
        
        sync_nodes(self.is_ddp)
        
        if self.rank==0: remove_file(self.output_temp_name)
            
        self.log(f"Inference completed successfully. Total inference time = {time()-self.run_stamp:.3f}s")
