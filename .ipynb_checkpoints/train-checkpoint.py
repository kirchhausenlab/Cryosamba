...
from core.utils.utils import setup_run, load_json, logger_info, set_writer_train, listify
from core.utils.data_utils import get_data
from core.utils.torch_utils import count_model_params, setup_DDP, sync_nodes, cleanup, save_ckpt, load_ckpt, get_lr, get_optimizer, get_scheduler
...
class Train:
    def __init__(self, args):
        ...
        self.model = get_model(cfg, self.device, is_ddp=self.is_ddp, compile=self.compile)
        self.optimizer = get_optimizer(self.model, cfg.optimizer)
        self.scheduler = get_scheduler(self.optimizer, cfg.train.warmup_iters, cfg.optimizer.lr_decay)
        ...
        self.train_loader = get_dataloader(cfg.train_data, data_list, metadata_list, split="train", is_ddp=self.is_ddp, shuffle=True)
        self.val_loader = get_dataloader(cfg.train_data, data_list, metadata_list, split="val", is_ddp=False, shuffle=False)
        
        self.log(f'# of data samples = {len(self.train_loader)} (train), {len(self.val_loader)} (val)')

    def log(self, message):
        return logger_info(self.rank, message)

    def print_train_loss(self, train_loss, print_time):
        save_ckpt(self.model, self.optimizer, self.scheduler, self.iter, self.resume_ckpt)
        ...
        message += ", ".join(f"{value:.6f} ({key})" for key, value in train_loss.items())
        message += f", E.T.: {(time()-print_time):.3f}s"
        self.log(message)

    def save_model(self):
        self.zfill = len(str(self.num_iters))
        save_ckpt_path = os.path.join(self.train_dir, str(int(self.iter)).zfill(self.zfill)+'.pt' )
    ...
    def validation(self):
        ...
        self.log(f"Iter {self.iter}/{self.num_iters} :: val loss: {val_loss:.6f}, E.T.: {val_time:.3f}s")
    
    def run_training(self):

        # Load checkpoint

        self.model, self.optimizer, self.scheduler, self.iter = load_ckpt(self.resume_ckpt, self.model, self.optimizer, self.scheduler, is_ddp=self.is_ddp, compile=self.compile)
        if self.load_ckpt_path is not None:
            self.model = load_ckpt(self.load_ckpt_path, self.model, is_ddp=self.is_ddp, compile=self.compile)[0]
    
        # Training loop
           
        if self.rank == 0: train_loss = {f'gap {t}': 0 for t in range(1, self.max_frame_gap + 1)}
        ...
        while self.iter <= self.num_iters:
            ...
            self.model.train()
            if self.is_ddp: self.train_loader.sampler.set_epoch(epoch)
            ...
            for i, imgs in enumerate(self.train_loader):       
                ...
                for t in range(1, self.max_frame_gap+1):
                    ...
                    if self.rank == 0: train_loss[f'gap {t}'] += float(loss.detach().item())/self.print_freq
                ...
                if self.rank == 0 and self.iter>0:
                    ...
                    if (self.iter % self.val_freq == 0): self.validation()
                        
                    if (self.iter % self.save_freq == 0): self.save_model()
    
                    if (self.iter % self.print_freq == 0): print_time = time()
        ...
        self.log(f"Training completed successfully. Total training time = {time()-self.run_stamp:.3f}s")
