    ...
    def test_config_file_permissions(self):
        try:
            self.assertTrue(
                ...
                "Inference config file is not readable",
            )
            self.assertTrue(
                os.access(self.config_path, os.W_OK),
                "Inference config file is not writable",
            )
        except Exception as e:
            logger.error(
                "❌ Error checking inference config file permissions: %s", str(e)
            )
    ...
    def test_config_file_format(self):
        try:
            with open(self.config_path, "r") as f:
                config = json.load(f)
            self.assertIsInstance(
                config, dict, "Inference config file is not a valid JSON dictionary"
            )
