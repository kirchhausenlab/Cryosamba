...
logging.basicConfig(filename='debug_errors_for_inference.log', encoding='utf-8', level=logging.DEBUG)
...
def generate_mandatory_params():
    ...
    with st.container():
        ...
        st.markdown("_The name of the folder where the checkpoints were saved (exp-name/train) in your training run._")

        st.markdown("**Data Path**")
        data_path = st.text_input("data_path", "/nfs/datasync4/inacio/data/raw_data/cryo/novareconstructions/rotacell_grid1_TS09_ctf_6xBin.rec")
        st.markdown("_Filename (for single 3D file) or folder (for 2D sequence) where the raw data is located._")

        st.markdown("**Inference Directory Path**")
        inference_dir = st.text_input("inference_dir", "/nfs/datasync4/inacio/data/denoising/cryosamba/rota/inference/")
        st.markdown("_The name of the folder where the denoised stack will be saved (exp-name/inference)._")

        st.markdown("**Maximum Frame Gap**")
        max_frame_gap = st.slider("inference_data.max_frame_gap", min_value=1, max_value=40, value=12)
        st.markdown("_The maximum frame gap used for inference (usually two times the value used for training). Explained in the manuscript._")
...
def generate_additional_params():
    ...
    with st.container():
        st.button("Inference Data Parameters", on_click=lambda: st.session_state.update({"additional_params_section": "inference_data"}))
        st.button("Inference Parameters", on_click=lambda: st.session_state.update({"additional_params_section": "inference"}))
    ...
    if additional_params_section == "inference":
        ...
        st.markdown("_If true uses test-time augmentation, which makes results slightly better but takes much longer (around 3x) to run._")
...
def generate_config():
    ...
    inference_data_params = {**inference_data_defaults, **st.session_state.get("inference_data_params", {})}
    inference_params = {**inference_defaults, **st.session_state.get("inference_params", {})}
