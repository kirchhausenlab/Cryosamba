...
logging.basicConfig(filename='main_error_log.log', encoding='utf-8', level=logging.DEBUG)

def main():
    st.sidebar.title("Cryosamba Navigation")
    app_mode=st.sidebar.selectbox("Choose the app mode", ["Choose your options!", "Setup Environment", "Setup Training", "Setup Inference", "Run Training", "Run Inference"])
    if app_mode=="Choose your options!":
        st.header("Please look at the options from the dropdown to either setup, train or run inferences")
