...
def install_conda():
    ...
    if sys.platform.startswith('linux'):
        ...
        run_command("wget https://repo.anaconda.com/miniconda/Miniconda3-latest-MacOSX-x86_64.sh")
        ...
        run_command("powershell -Command \"(New-Object Net.WebClient).DownloadFile('https://repo.anaconda.com/miniconda/Miniconda3-latest-Windows-x86_64.exe', 'Miniconda3-latest-Windows-x86_64.exe')\"")
        run_command("start /wait \"\" Miniconda3-latest-Windows-x86_64.exe /InstallationType=JustMe /AddToPath=1 /RegisterPython=0 /S /D=%UserProfile%\\Miniconda3")
    else:
...
def setup_environment_for_cryosamba() -> None:
    ...
    if not is_conda_installed():
        st.warning("Conda is not installed. You need to install Conda before proceeding.")
        if st.button("Install Conda"):
            if setup_conda():
                st.success("Conda installed successfully. Please restart the application.")
            else:
                st.error("Failed to install Conda. Please try again or install manually.")
    else:
...
def setup_environment(env_name):
    ...
    run_command(f"conda activate {env_name} && pip install torchvision torchaudio --index-url https://download.pytorch.org/whl/cu118")
    run_command(f"conda activate {env_name} && pip install tifffile mrcfile easydict loguru tensorboard streamlit pipreqs cupy-cuda11x")
