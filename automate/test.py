...
def run_command(command, shell=True):
    ...
    if process.returncode != 0:
        st.warning(f"Warning executing command: {command}\nError: {error}")
        logger.warning(f"Warning executing command: {command}\nError: {error}")
    return output, error
...
def setup_environment(env_name):
    ...
    packages = [
        ...
        "tifffile", "mrcfile", "easydict", "loguru", "tensorboard", "streamlit", "pipreqs", "cupy-cuda11x"
    ]
    
    for package in packages:
        if isinstance(package, tuple):
            package_name, extra_args = package
        else:
            package_name, extra_args = package, ""
        
        st.write(f"Installing {package_name}")
        output, error = run_command(f"conda run -n {env_name} pip install {package_name} {extra_args}")
        if error:
            st.warning(f"Warning while installing {package_name}: {error}")
            logger.warning(f"Warning while installing {package_name}: {error}")
        else:
