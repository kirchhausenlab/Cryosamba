...
logging.basicConfig(filename='debug_errors_for_training.log', encoding='utf-8', level=logging.DEBUG)
...
def generate_mandatory_params():
    ...
    with st.container():
        ...
        train_dir = st.text_input("train_dir", "/nfs/datasync4/inacio/data/denoising/cryosamba/rota/train/")
        st.markdown("_The name of the folder where the checkpoints will be saved (exp-name/train)._")

        st.markdown("**Data Path**")
        data_path = st.text_input("data_path", "/nfs/datasync4/inacio/data/raw_data/cryo/novareconstructions/rotacell_grid1_TS09_ctf_3xBin.rec")
        st.markdown("_Filename (for single 3D file) or folder (for 2D sequence) where the raw data is located. This can be a list, in case you want to train on several volumes at the same time._")

        st.markdown("**Maximum Frame Gap**")
        max_frame_gap = st.slider("train_data.max_frame_gap", min_value=1, max_value=20, value=6)
        st.markdown("_The maximum frame gap used for training. Explained in the manuscript._")
...
def generate_additional_params():
    ...
    with st.container():
        st.button("Train Data Parameters", on_click=lambda: st.session_state.update({"additional_params_section": "train_data"}))
        st.button("Train Parameters", on_click=lambda: st.session_state.update({"additional_params_section": "train"}))
        st.button("Optimizer Parameters", on_click=lambda: st.session_state.update({"additional_params_section": "optimizer"}))
        st.button("Biflownet Parameters", on_click=lambda: st.session_state.update({"additional_params_section": "biflownet"}))
        st.button("Fusionnet Parameters", on_click=lambda: st.session_state.update({"additional_params_section": "fusionnet"}))
    ...
    if additional_params_section == "train_data":
        ...
        st.markdown("_X and Y resolution of the patches the model will be trained on. Doesn't need to be square (resolution x = resolution y), but it has to be a multiple of 32._")
        st.markdown("_Number of data points loaded into the GPU at once. Increasing it makes the model train faster (with diminishing returns for large batch size), but requires more GPU memory. Play with it to avoid GPU out of memory errors._")
    ...
    if additional_params_section == "train":
        ...
        st.markdown("_If true, uses torch.compile for faster training, which is good but takes some minutes to start running the script and it’s somewhat buggy. Recommend using false until you’re comfortable with the code._")
        st.markdown("_Length of the training run. The default value (200k) is a very long time, but you can halt the training whenever you feel it’s fine (see Tensorboard below)._")
    ...
    if additional_params_section == "fusionnet":
        ...
        if st.button("Save Fusionnet Parameters"):
            st.session_state.fusionnet_params = {
...
def generate_config():
    ...
    train_data_params = {**train_data_defaults, **st.session_state.get("train_data_params", {})}
    train_params = {**train_defaults, **st.session_state.get("train_params", {})}
    optimizer_params = {**optimizer_defaults, **st.session_state.get("optimizer_params", {})}
    biflownet_params = {**biflownet_defaults, **st.session_state.get("biflownet_params", {})}
    fusionnet_params = {**fusionnet_defaults, **st.session_state.get("fusionnet_params", {})}
...
def setup_cryosamba_and_training() -> None:
    ...
    if 'DEFAULT_NAME' not in st.session_state:
        ...
        if step == "mandatory_params":
            ...
            st.success("Configuration already generated. No further modifications allowed.")
