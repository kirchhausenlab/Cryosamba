...
def run_experiment(gpus: str, folder_path: str) -> None:
    ...
    if typer.confirm("Run the command?"):
        ...
        rprint(
            "[grey]CUDA_VISIBLE_DEVICES=0,1 torchrun --standalone --nproc_per_node=2 train.py --config configs/your_config_train.json[/grey] \n\n"
        )
        ...
        print(
            f"Please open up a new terminal on your machine and navigate to the cryosamba/automate folder. Then run this command or you can hit Ctrl-C and run this command: \n"
        )
...
def make_train_inference_dir() -> None:
    ...
    while True:
        ...
        if input_cmd == "y":
            ...
            rprint(
                f"You currently have the following experiments {os.listdir(exp_path)}"
            )
...
def generate_configs(exp_path: str) -> None:
    ...
    typer.echo(
        "Configs generated and saved as train_config.json and inference_config.json"
    )
...
def main():
    ...
    print(
        "Please note that you need a GPU to run cryosamba. If you cannot see GPU information, your machine may not support cryosamba."
    )
    ...
    if os.path.exists(base_path):
        ...
        print(
            f"Folder {base_path} not found. Please check the experiment name and try again."
        )
