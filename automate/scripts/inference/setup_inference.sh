...
echo "Enter the max frame gap for the inference(usally 2 x value use for training, press Enter for default of 12)"
read -r max_frame_gap
max_frame_gap=${max_frame_gap:-12}

echo "Enter TTA (uses test-time augmentation for better results however is 3x slower to run), press enter for default:true)"
read -r TTA
TTA=${TTA:-true}

echo "set compile to true to use torch.compile however can cause some errors because of longer startup times (press enter for default:false)"
read -r compile
compile=${compile:-false}

config_file="../../../$EXP_NAME/inference_config.json"
